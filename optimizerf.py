...
import argparse
#print("nano =", datetime.now().strftime("%H:%M:%S"))
from nanovna import NanoVNAV2
#print("nanovna =", datetime.now().strftime("%H:%M:%S"))
import skrf
...
#print("mpl =", datetime.now().strftime("%H:%M:%S"))
import time
from datetime import timedelta
import threading
import os
from skopt import gp_minimize
#print("skopt =", datetime.now().strftime("%H:%M:%S"))
from scipy import interpolate
import ast
from tkinter import *
import sys
#print("01, Current Time =", datetime.now().strftime("%H:%M:%S"))
...
def optimize(f_start, f_stop, sweep_pts, cal, load_cal, f_goal, goal, s_parameter, parameters, bounds, reset, max_calls, exp_ratio, conv_ratio, output):
	...
	if load_cal != '':
		...
		if cal:
			...
			ntw_init.write_touchstone(load_cal+'/open', skrf_comment=False)
			...
			ntw_init.write_touchstone(load_cal+'/short', skrf_comment=False)
			...
			ntw_init.write_touchstone(load_cal+'/load', skrf_comment=False)
			...
			ntw_init.write_touchstone(load_cal+'/thru', skrf_comment=False)
			calibrated = True
	if calibrated:
		### Create ideals ###
		...
		my_measured = [
			skrf.Network(load_cal+'/short.s2p'),
		...
		calibration = skrf.calibration.TwoPortOnePath(
	...
	if len(bounds) == 2:
		grid_x, grid_y = np.mgrid[bounds[0][0]:bounds[0][1]:250j, bounds[1][0]:bounds[1][1]:250j]
		...
		try:
			...
			with open('log.txt', 'a') as log:
				...
				for parameter_idx in range(len(parameters)):
					log.write('[*] Shifting x'+str(parameter_idx+1)+' to position: '+str(goto[parameter_idx])+'\n')
			...
			plt.pause(0.01)
			plt.clf()
			#print("b, Current Time =", datetime.now().strftime("%H:%M:%S"))

			#for plt_thread in plt_threads:
			#kil	threads.append(threading.Thread(target=plt_thread, args=(0,)))
			#print("b2, Current Time =", datetime.now().strftime("%H:%M:%S"))
			for parameter_idx in range(len(parameters)):
				threads.append(threading.Thread(target=modules[parameter_idx].drive, args=(goto[parameter_idx],)))
			#print("c, Current Time =", datetime.now().strftime("%H:%M:%S"))
			for thread in threads:
				thread.start()
			for thread in threads:
				thread.join()
			#print("d, Current Time =", datetime.now().strftime("%H:%M:%S"))
			...
			if flag:
				...
				ntw_best.write_touchstone('Best', skrf_comment=False)
				ntw_best = skrf.Network('Best.s2p')
			elif sign*skrf.mathFunctions.complex_2_db(s[idx,1,0]) < best:
				best = sign*skrf.mathFunctions.complex_2_db(s[idx,1,0])
				ntw_best = ntw
				ntw_best.write_touchstone('Best', skrf_comment=False)
				ntw_best = skrf.Network('Best.s2p')
			...
			for parameter_idx in range(len(parameters)):
				ax3.plot(range(1, len(positions)+1), positions[:,parameter_idx], label='$x_{'+str(parameter_idx+1)+'}$')
			ax3.set_xlim(left=1)
			ax3.grid()
			if len(positions) < 2:
				ax3.set_xticklabels([])
			...
			ax4.plot(range(1, len(s11_progress)+1),s11_progress,label='$S_{11} \ \mathrm{('+f_goal_label+' \ \mathrm{MHz})}$')
			ax4.plot(range(1, len(s21_progress)+1),s21_progress,label='$S_{21} \ \mathrm{('+f_goal_label+' \ \mathrm{MHz})}$')
			ax4.set_xlim(left=1)
			...
			if len(bounds) == 2:
				...
				if not flag and len(positions) >= 4:
					...
					ax5.imshow(grid_z2.T, extent=(bounds[0][0],bounds[0][1],bounds[1][0],bounds[1][1]), origin='lower')
					levels = np.arange(np.amin(s21_progress), np.amax(s21_progress), abs(np.amax(s21_progress)-np.amin(s21_progress))/3)
					ax5.contour(grid_z2.T, levels, colors='k', origin='lower', extent=(bounds[0][0],bounds[0][1],bounds[1][0],bounds[1][1]))
					ax5.plot(positions[:,0], positions[:,1], 'r.', ms=3)
					ax5.set_aspect((bounds[0][1]-bounds[0][0])/(bounds[1][1]-bounds[1][0]), anchor='N')
				ax5.set_xlabel('$x_{1}$')
				ax6.set_xticks([])
				...
				if not flag and len(positions) >= 4:
					...
					ax6.imshow(grid_z2.T, extent=(bounds[0][0],bounds[0][1],bounds[1][0],bounds[1][1]), origin='lower')
					levels = np.arange(np.amin(s11_progress), np.amax(s11_progress), abs(np.amax(s11_progress)-np.amin(s11_progress))/3)
					ax6.contour(grid_z2.T, levels, colors='k', origin='lower', extent=(bounds[0][0],bounds[0][1],bounds[1][0],bounds[1][1]))
					ax6.plot(positions[:,0], positions[:,1], 'r.', ms=3)
					ax6.set_aspect((bounds[0][1]-bounds[0][0])/(bounds[1][1]-bounds[1][0]), anchor='S')
			...
			fig.text(x=0.850, y=0.926, s=r'$\mathrm{\underline{Optimize\textbf{RF}}}$', fontsize=26, color='seagreen')

			y_offset = 0
			if len(bounds) != 2:
				y_offset = 0.6
			
			fig.text(x=0.847, y=0.160+y_offset, s=r'$\mathrm{\underline{Optimization \ Status}}$', fontsize=16)
			...
			fig.text(x=0.820, y=0.125+y_offset, s=r'$\mathrm{Search \ Space}$: $\mathrm{'+search_space+'}$', fontsize=14)
			...
			if len(s_progress) > int(exp_ratio*max_calls):
				...
				else: # Minimize magnitude
					last_best_idx = np.argmin(s_progress_reversed)
				if last_best_idx >= int(conv_ratio*max_calls):
					convergence = 'Reached'

			fig.text(x=0.839, y=0.095+y_offset, s=r'$\mathrm{Convergence}$: $\mathrm{'+convergence+r'}$', fontsize=14)

			percentage = str(round(100*(len(positions)/max_calls), 1))
			fig.text(x=0.844, y=0.065+y_offset, s=r'$\mathrm{Progress}$: $\mathrm{'+str(len(positions))+r'/'+str(max_calls)+r'\ ('+percentage+r'\%)}$', fontsize=14)

			runtime = int(time.time()-start_time)
			fig.text(x=0.866, y=0.035+y_offset, s=r'$\mathrm{Runtime}$: $\mathrm{'+str(timedelta(seconds=runtime))[:-3].replace(':', r'}$:$\mathrm{')+r'}$', fontsize=14)
			...
			#print("4, Current Time =", datetime.now().strftime("%H:%M:%S"))
			#plt.show()
			#plt.draw()
			#plt.pause(0.01)
			#plt.clf()
			#print("6, Current Time =", datetime.now().strftime("%H:%M:%S"))
		except KeyboardInterrupt:
			raise ValueError('\n[-] Sweep terminated by user.')

		if s_parameter.lower() == 's21':
			magnitude = sign*skrf.mathFunctions.complex_2_db(s[idx,1,0])
		else:
			magnitude = sign*skrf.mathFunctions.complex_2_db(s[idx,0,0])

		#print("10, Current Time =", datetime.now().strftime("%H:%M:%S"))
	...
	try:
		...
		s_mag([0] * len(bounds))
		# Begin optimization
		result = gp_minimize(s_mag, bounds, n_calls=max_calls, n_initial_points=int(exp_ratio*max_calls), initial_point_generator='lhs', verbose=True)
	...
	if reset:
		...
		threads = []
		for parameter_idx in range(len(parameters)):
			threads.append(threading.Thread(target=modules[parameter_idx].drive, args=(0,)))
...
def main(gui=False):
	...
	if gui:
		...
		parser.set_defaults(gui=False)
		# Sweep arguments
		parser.add_argument('-f0', '--f_start', dest='f_start',
							help='Start frequency (Hz)', type=float, required=True)
		parser.add_argument('-f1', '--f_stop', dest='f_stop',
							help='Stop frequency (Hz)', type=float, required=True)
		parser.add_argument('-t', '--sweep_pts', dest='sweep_pts',
							help='Sweep points (Hz)', type=int, default=101)
		# Measurement calibration
		parser.add_argument('-c', '--cal', dest='cal', action='store_true',
							help='Perform interactive measurement calibration')
		parser.set_defaults(cal=False)
		parser.add_argument('-l', '--load_cal', dest='load_cal',
							help='Import local measurement calibration from directory', type=str, default='')
		# Parameter properties
		parser.add_argument('-p', '--parameters', dest='parameters', nargs='+',
							help='Tuning parameters (module names)', type=str, required=True)
		parser.add_argument('-b', '--bounds', dest='bounds',
							help='Parameters boundaries. Format: \'[(x1_min, x1_max), (x2_min, x2_max), ..., (xn_min, xn_max)]\'', type=str, required=True)
		parser.add_argument('-r', '--reset', dest='reset', action='store_true',
							help='Reset parameters back to initial positions')
		parser.set_defaults(reset=True)
		# Goal arguments
		parser.add_argument('-f', '--f_goal', dest='f_goal',
							help='Target frequency (Hz)', type=float, required=True)
		parser.add_argument('-g', '--goal', dest='goal',
							help='Goal operator (minimize/maximize)', type=str, default='max')
		parser.add_argument('-s', '--s_parameter', dest='s_parameter',
							help='Scattering parameter\'s magnitude to optimize', type=str, default='S21')
		# Optimizer settings
		parser.add_argument('-i', '--max_calls', dest='max_calls',
							help='Maximum number of function iterations (calls)', type=int, default=100)
		parser.add_argument('-e', '--exp_ratio', dest='exp_ratio',
							help='Exploration ratio (initial global search)', type=float, default=0.1)
		parser.add_argument('-q', '--conv_ratio', dest='conv_ratio',
							help='Convergence ratio', type=float, default=0.3)
		# Result output
		parser.add_argument('-o', '--output', dest='output',
							help='Results output filename', type=str, default='')
	...
	if not gui:
		...
		def start_optimizer():
			...
			Message(top, text='Starting Optimizer...', font=('Helvetica', 18, 'italic'), padx=50, pady=50).pack()
		...
		### Sweep arguments ###
		sweep_args_label = Label(app, text='Sweep Arguments', font=('Helvetica', 18, 'bold', 'underline'))
		sweep_args_label.grid(row=0, column=0, columnspan=2, pady=10, padx=10, sticky=W)
		# f_start
		f_start_text = DoubleVar()
		f_start_label = Label(app, text=' Start frequency: ', font=('Helvetica', 14))
		...
		f_stop_label = Label(app, text=' Stop frequency: ', font=('Helvetica', 14))
		...
		sweep_pts_label = Label(app, text=' Sweep Points: ', font=('Helvetica', 14))
		sweep_pts_label.grid(row=3, column=0, padx=20, sticky=W)
		sweep_pts_entry = Entry(app, textvariable=sweep_pts_text, width=12)
		sweep_pts_entry.grid(row=3, column=1)

		### Measurement Calibration ###
		measurement_calibration_label = Label(app, text='Measurement Calibration', font=('Helvetica', 18, 'bold', 'underline'))
		measurement_calibration_label.grid(row=4, column=0, columnspan=3, pady=10, padx=10, sticky=W)
		var_cal = IntVar()
		# cal
		cal = Radiobutton(app, text='Perform interactive calibration', variable=var_cal, value=1, font=('Helvetica', 14))
		cal.grid(row=5, column=0, columnspan=3, padx=10, sticky=W)
		# load_cal
		load_cal_text = StringVar(value='measured')
		load_cal = Radiobutton(app, text='Import local calibration from dir:', variable=var_cal, value=2, font=('Helvetica', 14))
		...
		# uncalibrated
		uncalibrated = Radiobutton(app, text='None (uncalibrated)', variable=var_cal, value=3, font=('Helvetica', 14))
		uncalibrated.grid(row=7, column=0, columnspan=3, padx=10, sticky=W)
		uncalibrated.select()

		### Parameters ###
		params_label = Label(app, text='Parameter Properties', font=('Helvetica', 18, 'bold', 'underline'))
		params_label.grid(row=8, column=0, columnspan=3, pady=10, padx=10, sticky=W)
		# parameters
		parameters_text = StringVar(value='module_x1 module_x2')
		parameters_label = Label(app, text=' Parameters:', font=('Helvetica', 14))
		parameters_label.grid(row=9, column=0, padx=20, sticky=W)
		parameters_entry = Entry(app, textvariable=parameters_text, width=32)
		parameters_entry.grid(row=9, column=1, columnspan=3)
		# boundaries
		bounds_text = StringVar(value='[(x1_min, x1_max), (x2_min, x2_max)]')
		bounds_label = Label(app, text=' Boundaries:', font=('Helvetica', 14))
		...
		# reset
		var_reset = IntVar()
		reset = Checkbutton(app, text='Reset parameters back to initial positions', variable=var_reset, font=('Helvetica', 14))
		reset.grid(row=11, column=0, columnspan=11, padx=10, sticky=W)
		reset.select()

		### Goals ###
		goals_label = Label(app, text='Optimization Goal', font=('Helvetica', 18, 'bold', 'underline'))
		goals_label.grid(row=12, column=0, columnspan=3, pady=10, padx=10, sticky=W)
		# f_goal
		f_goal_text = DoubleVar()
		f_goal_label = Label(app, text=' Target frequency: ', font=('Helvetica', 14))
		...
		goal_label = Label(app, text=' Optimization goal:', font=('Helvetica', 14))
		...
		objective_label = Label(app, text=' Objective S-Parameter:', font=('Helvetica', 14))
		objective_label.grid(row=15, column=0, padx=20, sticky=W)
		objective_entry = OptionMenu(app, objective_text, '     |S11|  ', '     |S21|  ')
		...
		### Optimizer Settings ###
		settings_label = Label(app, text='Optimizer Settings', font=('Helvetica', 18, 'bold', 'underline'))
		settings_label.grid(row=16, column=0, columnspan=3, pady=10, padx=10, sticky=W)
		# max_calls
		max_calls_text = IntVar(value=100)
		max_calls_label = Label(app, text=' Max calls: ', font=('Helvetica', 14))
		max_calls_label.grid(row=17, column=0, padx=20, sticky=W)
		max_calls_entry = Entry(app, textvariable=max_calls_text, width=12)
		max_calls_entry.grid(row=17, column=1)
		# exploration_ratio
		exploration_ratio_text = IntVar(value=15)
		exploration_ratio_label = Label(app, text=' Exploration ratio: ', font=('Helvetica', 14))
		exploration_ratio_label.grid(row=18, column=0, padx=20, sticky=W)
		exploration_ratio_entry = Entry(app, textvariable=exploration_ratio_text, width=12)
		exploration_ratio_entry.grid(row=18, column=1)
		exploration_ratio_label_percent = Label(app, text='%', font=('Helvetica', 14))
		exploration_ratio_label_percent.grid(row=18, column=2, sticky=W)
		# convergence_ratio
		convergence_ratio_text = IntVar(value=25)
		convergence_ratio_label = Label(app, text=' Convergence ratio: ', font=('Helvetica', 14))
		convergence_ratio_label.grid(row=19, column=0, padx=20, sticky=W)
		convergence_ratio_entry = Entry(app, textvariable=convergence_ratio_text, width=12)
		convergence_ratio_entry.grid(row=19, column=1)
		convergence_ratio_label_percent = Label(app, text='%', font=('Helvetica', 14))
		convergence_ratio_label_percent.grid(row=19, column=2, sticky=W)

		### Results ###
		results_label = Label(app, text='Results', font=('Helvetica', 18, 'bold', 'underline'))
		results_label.grid(row=20, column=0, columnspan=3, pady=10, padx=10, sticky=W)
		# output
		output_text = StringVar(value='results.pdf')
		output_label = Label(app, text=' Output filename:', font=('Helvetica', 14))
		...
		fullscreen = Checkbutton(app, text='Enable fullscreen', variable=var_fullscreen, font=('Helvetica', 14))
		fullscreen.grid(row=22, column=0, columnspan=11, padx=10, sticky=W)
		fullscreen.select()
		start = Button(app, text='Start Optimizer', command = start_optimizer, width=59, bg='#2edc71')
		start.grid(row=23, column=0, columnspan=3, padx=10, sticky=W, pady=10)
